<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12 relative z-10">
    <x-ui.heading-1 :title="$songId ? __('Edit Song') : __('Create New Song')" :description="$songId ? __('Update the song details') : __('Add a new song to the repertoire')" />

    <flux:card>
        <div class="p-6 sm:p-8">
            <form wire:submit="save" class="space-y-6">
                <flux:input wire:model="name" :label="__('Name')" placeholder="e.g. Wonderwall" required />

                <div class="grid grid-cols-2 gap-4">
                    <flux:input wire:model="artist" :label="__('Artist')" placeholder="e.g. Queen" required />

                    <flux:input wire:model="year" type="number" :label="__('Year')" placeholder="e.g. 1995"
                        min="1900" max="2100" />
                </div>

                <flux:textarea wire:model="description" :label="__('Description (Optional)')"
                    :placeholder="__('Additional notes about the song...')" rows="3" />
                ...
                <div class="flex justify-between items-center pt-4 border-t border-zinc-200 dark:border-zinc-700">
                    ...
                    <flux:button type="submit" variant="primary">
                        {{ $songId ? __('Update Song') : __('Create Song') }}
                    </flux:button>
                </div>
            </form>
        </div>
    ...
    @if ($songId)
        <div class="mt-8">
            <flux:card>
                <div class="p-6 sm:p-8">
                    <flux:label>{{ __('Sheets') }}</flux:label>
                    <flux:subheading class="mb-4">
                        {{ __('Manage the music sheets for this song, one per instrument part') }}
                    </flux:subheading>

                    @if ($sheets->isEmpty())
                        <p class="text-sm text-zinc-500 dark:text-zinc-400 italic mb-6">
                            {{ __('No sheets yet. Upload the first one below.') }}
                        </p>
                    @else
                        <div class="space-y-2 mb-6">
                            @foreach ($sheets as $sheet)
                                <x-ui.removable-row wire:key="sheet-{{ $sheet->id }}"
                                    ...
                                    :confirmHeading="__('Remove sheet')" :confirmMessage="__('The PDF file will be permanently deleted.')">
                    ...
                    <div class="space-y-4 pt-4 border-t border-zinc-200 dark:border-zinc-700">
                        <flux:subheading>{{ __('Add a sheet') }}</flux:subheading>

                        <div class="grid grid-cols-2 gap-4">
                            <flux:field>
                                <flux:label>{{ __('Part') }}</flux:label>
                                <flux:select wire:model="newSheetPartId" :placeholder="__('Select a part...')">
