<div>
    ...
    <flux:modal wire:model="showViewModal" class="md:w-[600px]">
        @if ($viewingRecord)
            ...
            {{-- Gigs --}}
            @if ($viewingRecord->gigs->isNotEmpty())
                <div class="mb-6">
                    <h3 class="text-sm font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                    ...
                    <div class="flex flex-col gap-2">
                        @foreach ($viewingRecord->gigs->sortByDesc('date') as $gig)
                            <a href="{{ route('gigs.index', ['view' => $gig->id]) }}"
                               wire:navigate
                               class="flex items-center gap-3 p-3 rounded-lg border border-zinc-200 dark:border-zinc-700 hover:border-amber-400 dark:hover:border-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/10 transition-all group">
                                <div class="w-9 h-9 rounded-lg bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center flex-shrink-0 group-hover:bg-amber-200 dark:group-hover:bg-amber-900/50 transition-colors">
                                    <flux:icon.calendar class="size-5 text-amber-600 dark:text-amber-400" />
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="font-medium text-sm text-zinc-900 dark:text-zinc-100 truncate">{{ $gig->name }}</p>
                                    <p class="text-xs text-zinc-500 dark:text-zinc-400">
                                        {{ $gig->date->format('d.m.Y') }}@if ($gig->location) · {{ $gig->location }}@endif
                                    </p>
                                </div>
                            </a>
                        @endforeach
            ...
            @else
                <div class="mb-6 p-4 rounded-lg bg-zinc-50 dark:bg-zinc-800/50 text-center">
                    <p class="text-sm text-zinc-500 dark:text-zinc-400 italic">{{ __('Not played at any gig yet') }}</p>
                </div>
            @endif
